...
ServerManager::ServerManager(ConfigParser::Server server_conf, int socket) {
	...
	this->pipe_fd = -1; // Inicializar pipe_fd como valor inválido
}

ServerManager::~ServerManager() {}

// Método para obtener el descriptor del pipe
int ServerManager::getPipeFd() const {
    return pipe_fd;
}

// Método para establecer el descriptor del pipe
void ServerManager::setPipeFd(int fd) {
    pipe_fd = fd;
}
...
std::string ServerManager::getFile(std::string request_path, std::string server_root, std::string cgi, std::string request) {
    ...
    if (path.find(".php") != std::string::npos && !cgi.empty()) {
        // Configurar el entorno para CGI
        unsetenv("CONTENT_TYPE");
        unsetenv("CONTENT_LENGTH");
        setenv("REQUEST_METHOD", "GET", 1);
        setenv("SCRIPT_FILENAME", path.c_str(), 1);
        ...
        if (pid == 0) {
            ...
            // Código padre: agregar el pipe a poll()
            this->setPipeFd(pipe_fd[0]);
            close(pipe_fd[1]);

            // En este punto no leemos del pipe, dejamos que poll() lo maneje
            // El bucle de poll() en el main se encargará de leer del pipe
            std::cout << "CGI iniciado y pipe agregado a poll" << std::endl;
            return "CGI iniciado, esperando respuesta...";  // Respuesta temporal
        }
    }

    // Manejando archivos normales (no CGI)
    int fd = open(path.c_str(), O_RDONLY);
    if (fd < 0) {
        return HTTP400 + this->active_server.error_pages[400];
    }
